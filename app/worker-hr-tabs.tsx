...
const WorkerHrTabs = forwardRef<WorkerHrTabsHandle, {contractId:number;kind:"salary"|"vacation"}>(function WorkerHrTabs({contractId,kind}, ref) {
  ...
  const save=async(body:any)=>{setMsg("");try{const r=await fetch("/api/hr",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)}),b=await r.json();setMsg(b.error||b.message);if(r.ok){if(body.action==="saveVacation")setVac(emptyVacation);await load()}return r.ok}catch{setMsg("Falha ao salvar as informações salariais.");return false}};
