...
async function saveCloudTaxEntries(companyLegacyId:number,month:string,period:string,result:Awaited<ReturnType<typeof calculateCloud>>){
 ...
 for(const item of needed)if(item.use&&!ids[item.kind]){const created=await supabaseAdmin.post<Row[]>("/rest/v1/services",{organization_id:config.organizationId,company_id:resolvedCompanyId,legacy_id:nextLegacy++,description:item.description,entry_type:"deduction",fgts_incidence:false,inss_incidence:false,irrf_incidence:false,affects_dsr:false,composes_production_average:false,active:true},{prefer:"return=representation"});ids[item.kind]=created[0]?.id;if(!ids[item.kind])throw new Error(`Não foi possível criar automaticamente o evento ${item.description}.`)}
