...
async function calculateCloud(
  ...
  const [contracts, services, monthlyEntries, brackets, dependentRows, rates,profiles] =
    await Promise.all([
      ...
      pagedGet(
        `/rest/v1/services?select=id,legacy_id,description,formula_code,inss_incidence,irrf_incidence,inss_13_incidence,fgts_13_incidence,composes_production_average&organization_id=eq.${config.organizationId}`,
      ),
      pagedGet(
  ...
  const resultRows = contracts
    .map((contract) => {
      ...
      const own = period==="vacation"?ownEntries.filter((entry)=>{const formula=String(serviceById.get(entry.service_id)?.formula_code||"");return formula.includes("[INSS_FERIAS]")||formula.includes("[IRPF_FERIAS]")}):period==="thirteenth"?ownEntries.filter((entry)=>{const service=serviceById.get(entry.service_id);return service?.inss_13_incidence||service?.fgts_13_incidence}):ownEntries.filter((entry)=>Number(serviceById.get(entry.service_id)?.legacy_id)!==120);
      ...
      const priorIrrf = period === "balance" ? contractMonthlyEntries.filter(entry=>entry.entry_date<`${month}-16`&&/IRRF|IMPOSTO.*RENDA/i.test(String(serviceById.get(entry.service_id)?.description||""))&&Number(entry.discount_cents)>0).reduce((sum,entry)=>sum+Number(entry.discount_cents),0):0;
