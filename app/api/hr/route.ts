  ...
  try {
    ...
    if (action === "saveProfile") {
      ...
      await db.prepare(`INSERT INTO worker_payroll_profiles (tenant_id,contract_id,employment_link_code,employment_link_description,contract_term,salary_type,base_salary_cents,daily_rate_cents,advance_rate_basis_points,updated_at)
        VALUES (?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(tenant_id,contract_id) DO UPDATE SET employment_link_code=excluded.employment_link_code,employment_link_description=excluded.employment_link_description,contract_term=excluded.contract_term,salary_type=excluded.salary_type,base_salary_cents=excluded.base_salary_cents,daily_rate_cents=excluded.daily_rate_cents,advance_rate_basis_points=excluded.advance_rate_basis_points,updated_at=CURRENT_TIMESTAMP`)
        .bind(t,contractId,String(b.linkCode||""),String(b.linkDescription||""),b.contractTerm==="determined"?"determined":"indefinite",b.salaryType==="daily"?"daily":"monthly",salary,daily,Math.round(Number(b.advanceRate||40)*100)).run();
