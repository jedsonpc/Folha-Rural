...
export async function cloudLaunchesGet(request: Request) {
  ...
  const month = url.searchParams.get("month") || brazilMonth();
  const reportOnly = url.searchParams.get("report") === "1";
  const access = await authorizeCloud(request, "Apontamentos", companyLegacyId);
  ...
  try {
    ...
    const [contracts, profiles, salaryHistory, services, entries, holidays] =
      await Promise.all([
    ...
    return Response.json({
      ...
      contracts: contracts.map((row) => {
        const profile = profiles.find(
            (profile) => profile.contract_id === row.id,
          ),
          latestSalary = salaryHistory
            .filter((item) => item.contract_id === row.id)
            .at(-1),
          baseSalaryCents = Number(
            latestSalary?.salary_cents || profile?.base_salary_cents || 0,
          ),
          monthly = profile?.salary_type === "monthly";
        return {
          id: row.id,
          ...
          baseSalaryCents,
          dailyRateCents: monthly
            ? Math.round(baseSalaryCents / 30)
            : Number(profile?.daily_rate_cents || 0),
        };
      }),
      ...
      entries: entries.map((row) => ({
        ...
        unitPriceMills:
          Number(row.quantity) > 0
            ...
            : Number(row.unit_price_cents) * 10,
  ...
  try {
    ...
    if (body.action === "deletePeriod") {
      if (!(await requireCloudAdmin(request)))
        return Response.json(
          { error: "Exclusão por período exclusiva do administrador." },
          { status: 403 },
        );
      const month = String(body.month || ""),
        deleteAll = body.deleteAll === true,
        selectedDays = Array.isArray(body.days)
          ? [...new Set(body.days.map(String))]
          : [];
      if (!/^20\d{2}-(0[1-9]|1[0-2])$/.test(month))
        return Response.json(
          { error: "Informe um mês válido para exclusão." },
          { status: 400 },
        );
      const validDays = selectedDays.filter(
        (day) =>
          /^20\d{2}-\d{2}-\d{2}$/.test(day) && day.startsWith(`${month}-`),
      );
      if (!deleteAll && !validDays.length)
        return Response.json(
          { error: "Selecione ao menos um dia do mês." },
          { status: 400 },
        );
      const companies = await supabaseAdmin.get<Array<{ id: string }>>(
          `/rest/v1/companies?select=id&organization_id=eq.${config.organizationId}&legacy_id=eq.${companyLegacyId}&limit=1`,
      ...
      return Response.json({
        ...
        message: `${rows.length} apontamento(s) excluído(s) do período.`,
      });
    }
    if (body.action === "clone") {
      const source = String(body.sourceDate || ""),
        target = String(body.targetDate || "");
      if (
        !/^20\d{2}-\d{2}-\d{2}$/.test(source) ||
        ...
        return Response.json(
          {
            error:
              "Informe duas datas válidas e diferentes, entre os anos 2000 e 2100.",
          },
          { status: 400 },
        );
      const companies = await supabaseAdmin.get<Array<{ id: string }>>(
      ...
      const sourceRows = await supabaseAdmin.get<Row[]>(
        `/rest/v1/daily_entries?select=id,contract_id,service_id,quantity,unit_price_cents,amount_cents,discount_cents,notes&organization_id=eq.${config.organizationId}&company_id=eq.${companyId}&entry_date=eq.${source}`,
      );
      if (!sourceRows.length)
        return Response.json(
          { error: "O dia de origem não possui lançamentos salvos." },
          { status: 404 },
        );
      await supabaseAdmin.post(
        "/rest/v1/daily_entries?on_conflict=organization_id,company_id,entry_date,contract_id,service_id",
        sourceRows.map((row) => ({
      ...
      return Response.json({
        ok: true,
        message: `${sourceRows.length} lançamentos clonados ou atualizados em ${target.split("-").reverse().join("/")}.`,
        affected: sourceRows.length,
      });
    }
    if (body.action === "updateEntry") {
      const id = String(body.id || ""),
        date = String(body.entryDate || ""),
        contractId = String(body.contractId || ""),
        serviceId = String(body.serviceId || ""),
        quantity = Number(body.quantity),
        unitMills = Math.round(Number(body.unitPrice) * 1000),
      ...
      await supabaseAdmin.patch(
        `/rest/v1/daily_entries?id=eq.${id}&organization_id=eq.${config.organizationId}&company_id=eq.${companyId}`,
        {
          entry_date: date,
          ...
          amount_cents: Math.round((quantity * unitMills) / 10),
        },
      );
    ...
    if (body.action === "delete") {
      ...
      if (!id)
        return Response.json(
          { error: "Lançamento inválido para exclusão." },
          { status: 400 },
        );
      const companies = await supabaseAdmin.get<Array<{ id: string }>>(
          `/rest/v1/companies?select=id&organization_id=eq.${config.organizationId}&legacy_id=eq.${companyLegacyId}&limit=1`,
      ...
      await supabaseAdmin.patch(
        `/rest/v1/daily_entries?organization_id=eq.${config.organizationId}&company_id=eq.${companyId}&cloned_from_id=eq.${id}`,
        { cloned_from_id: null },
      );
      await supabaseAdmin.delete(
        `/rest/v1/daily_entries?id=eq.${id}&organization_id=eq.${config.organizationId}&company_id=eq.${companyId}`,
      ...
      return Response.json({
        ok: true,
        message:
          "Lançamento excluído. Os apontamentos clonados foram preservados.",
      });
    }
    if (body.action === "save" || body.action === "saveBatch") {
      const date = String(body.entryDate || ""),
        rows =
          body.action === "saveBatch" && Array.isArray(body.rows)
            ? (body.rows as Array<Record<string, unknown>>)
            : [body];
      if (!/^20\d{2}-\d{2}-\d{2}$/.test(date) || !rows.length)
        return Response.json(
      ...
      const values = rows.flatMap((row) => {
        const contractId = String(row.contractId || ""),
          ...
          unit = Math.round(unitMills / 10);
        return !contractId || !serviceId || quantity <= 0 || unit < 0
          ? []
          : [
              {
                organization_id: config.organizationId,
      ...
      if (!values.length)
        return Response.json(
          { error: "Preencha colaborador, serviço, quantidade e valor." },
          { status: 400 },
        );
      await supabaseAdmin.post(
        "/rest/v1/daily_entries?on_conflict=organization_id,company_id,entry_date,contract_id,service_id",
      ...
      return Response.json({
        ...
        message:
          values.length === 1
            ? "Lançamento salvo."
            : `${values.length} apontamentos salvos na tabela.`,
